...
func resourceContainerGroupUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.HasChange("location") {
		model.Location = pointer.To(location.Normalize(d.Get("location").(string)))
	}

	if d.HasChange("sku") {
		model.Properties.Sku = pointer.To(containerinstance.ContainerGroupSku(d.Get("sku").(string)))
	}
	...
	if d.HasChange("init_container") || d.HasChange("container") {
		var containerGroupVolumes []containerinstance.Volume

		initContainers, initContainerVolumes, err := expandContainerGroupInitContainers(d, nil)
		if err != nil {
			return fmt.Errorf("expanding `init_container`: %+v", err)
		}
		model.Properties.InitContainers = initContainers
		if initContainerVolumes != nil {
			containerGroupVolumes = initContainerVolumes
		}

		containers, containerGroupPorts, containerVolumes, err := expandContainerGroupContainers(d, nil)
		if err != nil {
			return fmt.Errorf("expanding `container`: %+v", err)
		}
		model.Properties.Containers = containers
		if containerGroupVolumes != nil {
			containerGroupVolumes = append(containerGroupVolumes, containerVolumes...)
		}
		model.Properties.Volumes = &containerGroupVolumes

		if model.Properties.IPAddress != nil {
			model.Properties.IPAddress.Ports = containerGroupPorts
		}
	}

	if d.HasChange("os_type") {
		model.Properties.OsType = containerinstance.OperatingSystemTypes(d.Get("os_type").(string))
	}

	if d.HasChange("dns_config") {
		dnsConfig := d.Get("dns_config").([]interface{})
		model.Properties.DnsConfig = expandContainerGroupDnsConfig(dnsConfig)
	}

	if d.HasChange("Diagnostic") {
		diagnostics := d.Get("diagnostics").([]interface{})
		model.Properties.Diagnostics = expandContainerGroupDiagnostics(diagnostics)
	}

	if d.HasChange("ip_address_type") || d.HasChange("dns_name_label") || d.HasChange("dns_name_label_reuse_policy") {
		if model.Properties.IPAddress == nil {
			model.Properties.IPAddress = &containerinstance.IPAddress{}
		}
		model.Properties.IPAddress.Type = containerinstance.ContainerGroupIPAddressType(d.Get("ip_address_type").(string))
		model.Properties.IPAddress.DnsNameLabel = pointer.FromString(d.Get("dns_name_label").(string))
		model.Properties.IPAddress.AutoGeneratedDomainNameLabelScope = (*containerinstance.DnsNameLabelReusePolicy)(pointer.FromString(d.Get("dns_name_label_reuse_policy").(string)))

	}

	if d.HasChange("key_vault_key_id") || d.HasChange("key_vault_user_assigned_identity_id") {
		...
		if keyVaultKeyId != "" {
			...
			model.Properties.EncryptionProperties = &containerinstance.EncryptionProperties{
				VaultBaseUrl: keyId.KeyVaultBaseUrl,
				KeyName:      keyId.Name,
				KeyVersion:   keyId.Version,
			}
	...
	if d.HasChange("subnet_ids") {
		...
		model.Properties.SubnetIds = subnets
	}

	if d.HasChange("image_registry_credential") {
		model.Properties.ImageRegistryCredentials = expandContainerImageRegistryCredentials(d)
	}

	if d.HasChange("priority") {
		priority := d.Get("priority").(string)
		model.Properties.Priority = pointer.To(containerinstance.ContainerGroupPriority(priority))
	}

	if d.HasChange("restart_policy") {
		restartPolicy := containerinstance.ContainerGroupRestartPolicy(d.Get("restart_policy").(string))
		model.Properties.RestartPolicy = &restartPolicy
	}

	if d.HasChange("zones") {
		if d.Get("sku").(string) == "Confidential" {
			return fmt.Errorf("zones are not supported for Confidential SKU")
		}
